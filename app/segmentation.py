...
# Safely retrieve the Mapbox API Key
try:
    MAPBOX_API_KEY = st.secrets["MAPBOX_API_KEY"]
except KeyError:
    st.error("Mapbox API Key is missing. Please add it to secrets.toml or Streamlit Cloud settings.")
    MAPBOX_API_KEY = None

# Build Mapbox URL
if MAPBOX_API_KEY:
    MAPBOX_URL = (
        f"https://api.mapbox.com/styles/v1/mapbox/satellite-v9/tiles/{{z}}/{{x}}/{{y}}?access_token={MAPBOX_API_KEY}"
    )
else:
    MAPBOX_URL = None
...
def create_map(location, zoom_start=18):
    ...
    """
    if not MAPBOX_URL:
        st.error("Mapbox URL cannot be created. Mapbox API Key is missing.")
        return None
...
def show_prediction(image_path, selected_model):
    ...
    model = load_model_from_file(
        Path("C:/UHI_Project/data/models", MODELS[selected_model]["file_name"])
    )
...
def tab_live_segmentation():
    ...
    with col2:
        ...
        if not folium_map:
            return  # Exit if Mapbox URL couldn't be created
